...
import { DirsWatcher } from './watcher.mjs';

import data from 'akasharender/data.js';
data.init();
...
const config_module = await import(path.join(process.cwd(), options.config));
...
// TODO Make sure that content files are parsed for metadata from 
//    frontmatter or elsewhere, and that data is recorded into
//    the database
//
// TODO For example this script needs to search for files using
//    a given template.  That needs to be a find operation to
//    support that query.
...
// TODO:  For layouts change .. search for documents using that layout, and only rerender those files
// TODO:  For partials change .. render entire site
