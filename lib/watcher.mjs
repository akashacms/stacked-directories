...
// TODO implement the ignore/include stuff
...
export class DirsWatcher extends EventEmitter {
    ...
    async onChange(fpath, stats) {
        ...
        if (!vpinfo) {
            throw new Error(`onChange could not find mount point or vpath for ${fpath}`);
        }
    ...
    async onAdd(fpath, stats) {
        let vpinfo = this.vpathForFSPath(fpath);
        if (!vpinfo) {
            throw new Error(`onAdd could not find mount point or vpath for ${fpath}`);
        }
    ...
    async onUnlink(fpath) {
        let vpinfo = this.vpathForFSPath(fpath);
        if (!vpinfo) {
            throw new Error(`onUnlink could not find mount point or vpath for ${fpath}`);
        }
        let stack = await this.stackForVPath(vpinfo.vpath);
        if (stack.length === 0) {
